...
const FavoriteSubSessionCard = ({
  ...
  const handleOk = (e, val) => {
    if (val) {
      dispatch(
        deleteProfileData(dataTypes.favorites, { [type]: [`${favoriteId}`] })
      );
      setHide(true);
    }
    setShowModal(false);
  ...
  return (
    <Fragment>
      <div className={favoritesCardStyles.subSessionCard}>
...
export default FavoriteSubSessionCard;
